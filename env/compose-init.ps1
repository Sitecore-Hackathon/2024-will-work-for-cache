...
function InstallModule{
    ...
    $moduleInstalled = Get-InstalledModule -Name $ModuleName -RequiredVersion $ModuleVersion -ErrorAction SilentlyContinue
    if (-not $moduleInstalled) {
        Write-Host "Installing '$ModuleName'" -ForegroundColor Green
        Install-Module -Name $ModuleName -RequiredVersion $ModuleVersion -Repository $RepositoryName -Scope CurrentUser
    }
}
...
function Invoke-ComposeInit {
    ...
    if(![string]::IsNullOrEmpty($SpecificVersion)){
        $module = Find-Module -Name $moduleName -Repository $repositoryName -RequiredVersion $SpecificVersion
        $latestVersion = $module.Version
